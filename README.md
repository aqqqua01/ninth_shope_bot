...
3. Пользователь вводит базовую сумму в рублях
4. WebApp показывает сумму к оплате с комиссией (в рублях) и эквивалент в USDT
5. После подтверждения бот отправляет уведомление о том, что с пользователем свяжется оператор
6. Администратор видит заявку с полной информацией и кнопками управления
7. Администратор может изменить статус заявки или курс USDT через команды
...
В сообщениях о новых заявках администратор видит кнопки:

- **✅ Выполнено** - Завершить заявку успешно
- **🔄 В обработке** - Пометить заявку как обрабатываемую
- **❌ Отклонить** - Отклонить заявку с уведомлением пользователя
...
Бот логирует:
- Полученные заявки с суммами в РУБ и USDT
- Ошибки валидации
- Проблемы с отправкой в админ чаты
- Действия администратора с заявками
